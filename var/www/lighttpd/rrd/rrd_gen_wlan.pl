...
&ProcessInterface("wlan0", "V115r9 link");
...
#         $_[1]: interface description
	...
	my $signal = `iwconfig $_[0]|grep Signal|cut -d"=" -f3|cut -d" " -f1`;
	my $noise = `iwconfig $_[0]|grep Signal|cut -d"=" -f4|cut -d" " -f1`;
	#my $rate = `iwconfig $_[0]|grep Rate|cut -d"M" -f1|cut -d"=" -f2`;
	my $rate = `iwconfig $_[0]|grep Rate|cut -d"M" -f1|cut -b20-22`;

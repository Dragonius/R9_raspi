...
# Blue
for (0..127)   { @{$gradient[$_]} = ($_/2, $_/2 ,$_*1.5); }
for (128..255) { @{$gradient[$_]} = (64+ ($_-128)*1.5,  64+ ($_-128)*1.5, 192+($_-128)/2); }


open(S,"sox  \"".$opts{f}."\"  --norm -r $targetrate -b 16 -c 2 --multi-threaded  -t .raw -e signed - trim $skip_sec $gain $ygain|");
...
open(U,"|convert -depth 8  -size ".$w."x".int(65536/$yscale)." rgb:-  /var/www/html/image/".$random_number.".png");
